...
# TO RUN:
# python assignment_8.py --player1 human --player2 computer --timed yes
# python assignment_8.py --player1 human --player2 computer --timed no
...
class Player:
    ...
    def is_winner(self):
        return self.isWinnerVar

    def make_winner(self):
        self.isWinnerVar = True
    ...
    def roll_die(self):
        ...
        if current_roll == 1:
            print " Rolled: 1\n OOPS.. YOU HAVE LOST ALL YOUR POINTS, NEXT PLAYERS TURN.\n"
        elif current_roll == 2:
            print "Rolled: 2\nPoints this Round: %i" % self.tempScore
        elif current_roll == 3:
            print "Rolled: 3\nPoints this Round: %i" % self.tempScore
        elif current_roll == 4:
            print "Rolled: 4\nPoints this Round: %i" % self.tempScore
    ...
    def decide(self, rolled_value):
        ...
        if self.tempScore + self.scoreVar >= 100:
            print "Total points      : %i" % \
                  (self.tempScore + self.scoreVar)
            print "\n - PLAYER %i WINS THIS GAME WITH %i POINTS -" % ( self.identity(), self.tempScore + self.scoreVar)
            self.isWinnerVar = True
            return 0

        decision = raw_input("Type 'r' for roll and 'h' for hold. Decision: ")
...
class ComputerPlayer(Player):
    def decide(self, rolled_value):
        ...
        # logic behind computers move
        if (100 - self.scoreVar) >= 25:
            if self.tempScore < 25:
                decision = "r"
        else:
            if self.tempScore < (100 - self.scoreVar):
                decision = "r"
...
def main():
    ...
    parser.add_argument("--player1", help="Indicate if player 1 is 'human' or 'computer'.")
    parser.add_argument("--player2", help="Indicate if player 2 is 'human' or 'computer'.")
    parser.add_argument("--timed", help="Indicate if game is timed to 60 seconds. Type 'yes' or 'no'.")
    ...
    if timed_game == "yes":
        print "TIMED VERSION (60 seconds to play)\n"
